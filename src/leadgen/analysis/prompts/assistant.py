...
def _assistant_team_system_prompt(
    ...
    if crm:
        scope_label = {
            "own": "лиды собеседника (он селз)",
            "squad": "команда собеседника + свободный пул (он тимлид)",
            "company": "вся компания",
        }.get(crm.get("scope"), "вся компания")
        top_hot = "; ".join(
            f"{h['name']} ({h['score']})"
            for h in crm.get("top_hot") or []
        )
        running = "; ".join(crm.get("running_searches") or [])
        team_block += (
            "\n\n=============================================="
            "\nЖИВОЕ СОСТОЯНИЕ CRM — НА МОМЕНТ ЭТОГО СООБЩЕНИЯ\n"
            "==============================================\n"
            f"Разрез: {scope_label}.\n"
            f"- Лидов: {crm.get('total', 0)} "
            f"(в работе {crm.get('in_work', 0)}, "
            f"свободных {crm.get('free', 0)})\n"
            f"- Горячих (скор ≥ 75): {crm.get('hot', 0)}"
            + (f" — топ: {top_hot}" if top_hot else "")
            + "\n"
            f"- Просроченных перезвонов: "
            f"{crm.get('overdue_callbacks', 0)}\n"
            f"- Сегодня: наборов {crm.get('dials_today', 0)}, "
            f"целей {crm.get('goals_today', 0)}\n"
            + (f"- Сейчас собирается: {running}\n" if running else "")
            + f"- Токенов на балансе команды: "
            f"{crm.get('token_balance', 0)}\n"
            "Отвечая про текущее положение дел («как у нас дела», "
            "«что горит», «кому звонить»), опирайся на ЭТИ цифры — "
            "они настоящие и свежие. Не выдумывай других чисел. Если "
            "спрашивают глубже, чем видно отсюда, честно скажи, где "
            "в продукте посмотреть (CRM, Работа, Аналитика)."
        )
